...
VK-канари (закрыт 2026-09-01, живой BrowserOS через CDP :9223):
- BrowserOS держал unpacked из папки репо с кэшем загрузки 0.2.0 — правки
  файлов на диске не подхватывались; вылечено `chrome.runtime.reload()`
  из попап-контекста (стало 0.3.0).
- Захват был пуст ещё и из-за VK redesign 2026: у чатов больше нет href /
  dataset.peer; peer_id живёт в `data-itemkey="convo_<id>"` на
  `DIV.ConvoList__item`, у закреплённых `.FCThumb` — в id аватарной маски
  `#mePeerFrame…Mask-<id>`. selectors.js + chat-list.js обновлены.
- Результат: 25 чатов / 28 сообщений захвачено в реальном ВК; endpoint+token
  записаны в storage; форвард реального сообщения ушёл на прод;
  `GET /v1/conversations?source=vk` = 25 записей с реальным preview.
  Канал VK DOM → SW → UserIO prod работает непрерывно.
